...
def iterations(period, beta, InfectiousAgent, Infectedstate, HouseAgents, TotalHouse, InfectedAgentRec, InfectiondateRec, InfectionstateRec, p, k, LatentperiodRec, InfectiousperiodRec, Epsilon, Epsilon_asy, Gamma, beta_asy, beta_pre, mu, shoppingpoptranprob, pharmacypoptranprob, varying_mobility):
    ...
    for t in range(period):
        ...
        if t >= 34:
            Homeindex, Communityindex, workplaceindex, resturantindex, mallindex, marketindex = varying_mobility[t,5], (varying_mobility[t,5] - 90), (varying_mobility[t,4]-90), (varying_mobility[t,0]-90), (varying_mobility[t,0]-90), (varying_mobility[t,1]-90)
...
if __name__ == '__main__':
    ...
    simulationround = 1 # number of simulations
    ini_seednumber = np.random.randint(low=5, high=16) # Initial infectious inidividual
    R0 = 2.5 # Basic reproduction number 
    Epsilon = 3 # latent period for symptomatic cases
    Epsilon_asy = 5 # latent period for asymptomatic cases
    Gamma = 2 # pre-symptomatic period, i.e. Epsilon + Gamma = Epsilon_asy
    ...
    mu = 2.5 # time to removed, i.e. infectious period
    ...
    for fp in Totalcumulativenumber:
        np.savetxt('0331FinalLaptop3consle8'+str(i)+'.csv', fp, delimiter=',')
        i += 1
    # Totalcumulativeloc = np.array(TotallocExport)
    # i = 0
    # for fp in Totalcumulativeloc:
    #     np.savetxt('0331FinalLoclaptop1consle1'+str(i)+'.csv', fp, delimiter=',')
    #     i += 1